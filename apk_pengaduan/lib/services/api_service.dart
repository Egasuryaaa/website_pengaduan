...
class ApiService {
  ...
  Future<Map<String, dynamic>> login(String email, String password) async {
    ...
    try {
      return await retryWithAlternativeBaseUrl(() async {
        try {
          print('DEBUG: Making login request to: $baseUrl/login');
          print('DEBUG: With credentials - Email: $email, Password: [hidden]');
          ...
          // Handle different response formats from the Laravel API
          if (response.statusCode == 200 || response.statusCode == 201) {
            ...
            if (response.data['data'] != null && response.data['data']['token'] != null) {
              ...
              print('DEBUG: Login successful but no token found in response');
              return {
                'success': false,
            ...
            print('DEBUG: Login successful, saving token: ${token.substring(0, 10)}...');
  ...
  Future<Map<String, dynamic>> createPengaduan(dynamic data) async {
    try {
      // Check if we're authenticated before making the request
      final token = await getToken();
      print('DEBUG: Authentication token: ${token != null ? "Present" : "Missing"}');
      
      // Data sudah dalam bentuk FormData atau perlu dikonversi
      FormData formData;
      if (data is FormData) {
        formData = data;
        ...
        throw ArgumentError('Data must be FormData or Map<String, dynamic>');
      }
      
      print('DEBUG: Sending data to API: ${formData.fields}');
